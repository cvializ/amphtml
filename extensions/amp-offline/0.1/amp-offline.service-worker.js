...
const WorkerMessengerCommand = {
  /*
    Used to request the current offline saved state.
   */
  AMP_OFFLINE_QUERY_STATE: 'amp-offline-query-state',
  /*
    Enable offline mode
   */
  AMP_OFFLINE_SET_ENABLE: 'amp-offline-enable',
  /*
    Disable offline mode.
   */
  AMP_OFFLINE_SET_DISABLE: 'amp-offline-disable',
};
...
self.addEventListener('message', event => {
  ...
  switch (command) {
    case WorkerMessengerCommand.AMP_OFFLINE_QUERY_STATE:
      onMessageReceivedSubscriptionState();
      break;
    case WorkerMessengerCommand.AMP_OFFLINE_SET_ENABLE:
      onMessageReceivedSubscribe();
      break;
    case WorkerMessengerCommand.AMP_OFFLINE_SET_DISABLE:
...
/*
# TODO(cvializ):
- [ ] install service worker (the web push api doesn't seem to need install)
- [X] listen for messages
- [ ] enable offline for documents in the provided messages
- [ ] respond with errors if the provided documents cannot be cached
*/
